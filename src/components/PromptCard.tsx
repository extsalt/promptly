...
export function PromptCard({ prompt }: PromptCardProps) {
  ...
  return (
    <div className="glass glass-hover glossy-top-light transition-all rounded-lg p-5 flex flex-col gap-4">
      <div className="flex justify-between items-start">
        <div className="flex flex-col gap-1">
          <h3 className="text-lg font-bold text-foreground leading-tight">{prompt.title}</h3>
          <div className="flex items-center gap-2">
            {prompt.author && prompt.author !== 'Anonymous' ? (
              <Link href={`/profile/${prompt.author}`} className="text-xs font-semibold text-foreground/50 hover:text-primary transition-colors hover:underline">
                by @{prompt.author}
              </Link>
            ) : (
              <span className="text-xs font-semibold text-foreground/50">by Anonymous</span>
            )}
            <span className={`flex items-center gap-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full border ${badge.color}`}>
        ...
        <div className="flex gap-1.5 flex-wrap justify-end">
          {prompt.tags.map(tag => (
             <span key={tag} className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-foreground/5 text-foreground/70 border border-border/30 backdrop-blur-sm">
               {tag}
             </span>
          ))}
        </div>
      ...
      <div className="bg-foreground/[0.03] border border-border/20 rounded-lg p-4 relative group backdrop-blur-sm">
        <p className="text-foreground/80 font-mono text-sm leading-relaxed whitespace-pre-wrap line-clamp-3">
          {prompt.content}
        </p>
      </div>

      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-3 mt-2">
        <div className="flex items-center flex-wrap gap-2">
          <Button 
            variant="primary"
            size="sm"
            onClick={handleCopy}
            className="flex items-center gap-1.5 text-[11px] font-bold h-8 px-3 rounded-full shadow-sm active:scale-95 bg-foreground text-background hover:opacity-90 transition-opacity"
            title="Copy Prompt"
          >
            {copied ? (
          ...
          <div className="flex gap-1.5 flex-wrap">
            <a href={`https://chatgpt.com/?q=${encodeURIComponent(prompt.content)}`} target="_blank" rel="noopener noreferrer" className="text-[11px] font-medium px-2.5 py-1 bg-foreground/5 hover:bg-foreground/10 text-foreground transition-colors rounded-full border border-border/30 flex items-center gap-1.5 backdrop-blur-sm">
            ...
            <a href={`https://gemini.google.com/app?q=${encodeURIComponent(prompt.content)}`} target="_blank" rel="noopener noreferrer" className="text-[11px] font-medium px-2.5 py-1 bg-foreground/5 hover:bg-foreground/10 text-foreground transition-colors rounded-full border border-border/30 flex items-center gap-1.5 backdrop-blur-sm">
      ...
      <div className="flex items-center justify-between mt-1 pt-4 border-t border-border/20">
        <div className="flex gap-5">
          <button 
            onClick={handleUpvote}
            className={`flex items-center gap-1.5 text-sm font-semibold transition-colors ${hasUpvoted ? 'text-primary' : 'text-foreground/50 hover:text-primary'}`}
          >
             <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill={hasUpvoted ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m18 15-6-6-6 6"/></svg>
             {upvotes}
          </button>
          <button className="flex items-center gap-1.5 text-sm font-semibold text-foreground/50 hover:text-blue-500 transition-colors">
